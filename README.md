...
- Postgres/[Marten document DB](https://martendb.io/)
...
- Code: Seems I was in a hurry sometimes, some file names are wrong. Some files contain too many unrelated classes, or only comments or unused code. It's is a bit inconsistent, posted as is :-)
- Validation: the validation for the request hanlders is now implemented as a FluentValidation class per handler, it could have been a bit simpler by using a virtual validation method on the command handler itself if the few used methods were implemented in the project itself.
- MartenDB: I used the event projections to synchronize data with different external systems and to send emails. Projections can be monitored and rebuild via api (AdminController). It works in this case. A messagebus would have been necessary at some point.
...
- Event sourcing: I don't see a reason to 'event source' the aggregates (anymore). I don't dislike the design of the aggregates, but the command and event handler could be a single method if the aggregates wouldn't be 'event sourced'. And the single table event-store would probably have been the first bottleneck to solve performance wise. Raising the events from the aggregate still feels right, and the (event sourced) projections are definitly a huge advantage.
